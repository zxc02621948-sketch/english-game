...
- 六個以上字塊的複合回答改用寬版緊湊配置，答案格與字卡會自動換列，不再被內層捲軸及底部確認列裁切。
- 初階 `Are you ...?` 問答加入專用緊湊版面，縮短問題卡、聽音按鈕與答案格高度，避免只有三個字仍因上方內容過高而溢出。

## 2026-07-20

### 加入初階問答並降低後段重複（Codex）

- 新增初階完整回答題：看到並聽到 `Are you hungry?`，依中文提示排列 `I am hungry.`。
- 答題說明會指出問別人使用 `Are you ...?`，回答自己要換成 `I am ...`；同一組問答會進入句子冷卻。
- 後段動態關卡新增最近兩種題型冷卻，避免題型以 A→B→A 的方式反覆回來。
- 選字克漏與句子打字克漏現在都會記錄出過的完整句子，優先輪替其他內容；修正不限第 26 關，會套用到所有後續動態關卡。

### 避免 `eat rice` 連讀干擾拼字（Codex）

- 整句克漏字新增「單獨聽本課字」，遇到連讀不清楚時可把目標字拆出來聽。
- 若把 `rice` 寫成 `jice`，答錯說明會指出：`eat` 尾端的 `t` 接 `rice` 的 `r` 可能產生近似 `ch／j` 的連讀聽感，但拼字仍是 `rice`。

### 釐清分類題中的 `good` 與感受（Codex）

- 「感受」分類改名為「明確的心情／身體感受」，題目說明會先指出 `good／bad` 是較廣泛的評價。
- 若誤選 `good` 或 `bad`，答錯說明會補充：`I’m good.` 的確可以表示「我很好」，但這不代表 `good` 是特定心情名稱。

### 修正句意題的「這是／這個是」與中文量詞（Codex）

- 移除把英文逐字拼成「這個是一個書」之類中文的干擾答案；這類答案可能與正解語意相同，也會讓初學者記住不自然的中文。
- 可數名詞加入中文量詞，句型會產生「這是一隻貓／一本書／一棟房子／一位朋友」。
- `This is ...` 統一以自然的「這是……」呈現；不再把「這個是……」當成另一種英文文法或錯誤答案。

## 2026-07-18

### 日常前三關固定藍圖試驗（Codex）

- 日常第 1～3 關改用預先安排的學習順序，依序帶入 `cat`、`book`、`house`，避免同一題型因隨機抽選而連續重複。
- 第 1 關以教學、看圖、聽音與配對建立單字；第 2～3 關再加入 `This is a ...` 的組句與句意理解。
- 固定的是每關的主要學習節奏；答錯題仍會進入原本的補強佇列，熟練度、記憶間隔與提示機制也照常運作。
- 「我已經會了」會略過本關後續針對該字的單字題並移除待補考題，但仍保留句子中的整合應用。
- 試驗範圍只限日常第 1～3 關；日常第 4 關以後與工作英文暫時維持原本的動態出題，方便比較實際學習感受。
- 驗證：全部 JavaScript 通過 `node --check`；Node VM 確認題數為 8／7／7、指定句型可產生，且藍圖不會套用到日常第 4 關或工作英文。

### 本輪調整(Codex):舊內容回顧不再拿來灌滿關卡
- 前階、已學會且到期的純回顧內容改為每關最多 2 個,每個在主回合只占 1 題;當階內容、學習中內容、補默寫與答錯補考不受此限制。
- 日常與工作軌各自記最近兩關抽過的純舊內容,下一關優先換其他到期字,避免少數萬用字反覆出現。
- 題材不足時先補當階已會內容;只有純句型階或主動回玩舊關才開放較大的舊內容池,不再用任意舊字替一般新課湊長度。
- 重置全部進度時同步清除兩軌的舊題冷卻紀錄。

### 本輪調整(Codex):ea 明確作為記憶區塊
- `ea` 提示改成先把兩個字母視為一個拼讀／記憶區塊。
- 分成長音 i(`tea/eat/speak`)、短音 e(`bread/head/dead`)與字母 A 音(`great/break/steak`)三個小家族;移除容易受時態讀音干擾的 `read` 範例。
- 驗證:全 JS `node --check`、媒體引用存在檢查、三關舊題輪替模擬與 ea 三家族分類模擬皆通過。
...
## 2026-07-08(13 / Claude Opus 4.8)— 故事 ×3:狗狗咖啡店 / 貓不見了 / 會說話的狗(狗 = 全季貫穿角色)

### 加了什麼(純 STORIES 資料,引擎沒動)
- **階1 ☕ 狗狗咖啡店**:Sam 點咖啡,狗店員連上錯三次(茶→水→加鹽的咖啡),Sam 認命點水…又拿到茶。重組題 = 本階句型 Coffee with sugar。彩蛋:salt/sugar 的 why 鉤子。
- **階6 🐱 貓不見了**:Mia 找貓(Where is my cat? = 本階句型),狗走過來叫她「Look!」→ 貓就趴在狗背上。Bad cat! Good dog!(狗第一次被稱讚,嚇到)。
- **階8 🗣️ 會說話的狗**:路人問 Do you speak English?(本階句型 = 重組題),Bea 說會一點但對方講太快;路人開玩笑問狗 → 狗回 Hello! → 路人嚇跑。結尾狗:I speak English!
- **★ 狗 = 全季貫穿角色**:階1 上錯飲料的店員 → 階4 偷吃蛋糕嫁禍寶寶 → 階6 背上藏貓 → 階8 開口說話。角色連續性 = 便宜的「勾人」敘事(07-04 定案:勾人靠角色+反轉)。
...
## 2026-07-08(17 / Claude Opus 4.8)— 轉換題補考被切底:題目帶瘦身 + 句子題不塞複習鈕

### 使用者實玩
轉換題(補考時)下方字塊被動作列切掉:「這題太多東西了,我要複習這個字應該可以拿掉」。當下題目帶疊了四層:標題 + 原句卡(英/中/↓箭頭)+「我要複習這個字」鈕 + 答題帶的問句列 → 作答區被擠爆。

### 修(js/01 + js/06 + css/05)
- **句子題補考不塞「我要複習這個字」**(injectReviewButton 對 `sentence_*` 格式 return):複習卡是「單字卡」,對句子題幫助小、又把題目帶撐高。單字題補考照舊有;連錯 2 次的強制複習卡也照走。
- **原句卡拿掉「↓改成問句」箭頭行**(跟標題重複;答題帶還有 ❓問句 徽章當保險)+ 原句字級縮一階。
- **build-answer overflow visible → auto**:內容真的太高時寧可內部捲,也不被動作列硬切。
- **slots 列間距**收回一階(上一筆放太開)。
- 驗:轉換題補考無複習鈕、單字題有;箭頭行沒了;overflow auto。


## 2026-07-08(18 / Claude Opus 4.8)— 故事變成地圖上的一站(📖 故事節點)

### 使用者實玩
「情境題是不是只要沒有選擇要玩,之後在地圖上就看不到了?我覺得他應該當成一個關卡。」—— 對,原本只有結算卡一顆鈕(一閃即逝)+ 小遊戲 hub(藏太深)。

### 修(js/10 mapSVG + showHome)
- **地圖故事節點**:每個故事在「該階第 5 關」旁邊畫一顆紫色節點(虛線岔出主線),icon + 標題。
- **狀態**:該階沒完成 → 灰 38% + 🔒 標題、不可點(吊胃口);完成 → 亮、可點,讀完回地圖;讀過 → 節點右上 ✓。
- **只在日常軌畫**(STORIES 不分軌,工作軌 guard 掉);地圖還沒展開到該階第 5 關就不畫。
- 點擊音效 selector 加 .mapstorynode。結算卡鈕 + 小遊戲 hub 重讀入口都保留(三個入口)。
- 驗:新開局不畫 / 第1階進行中 = 鎖定(op .38 + 🔒 + 不可點)/ 完成該階 = 可點 / 讀完 = ✓ / 工作軌 0 個。


## 2026-07-08(19 / Claude Opus 4.8)— 故事節點疊在關卡上:改垂直路徑推出去

### 使用者實玩
故事節點(☕)疊在第 6 關節點上面。原因:放在第 5、6 關中點再「水平」推 100px → 那段路徑是斜的,水平推移不夠、就壓到相鄰關卡節點。

### 修(js/10 mapSVG)
- 改成**沿路徑法線方向**(垂直於 5→6 那段線)推出去 116px(compact 104)→ 斜線段也保證離開路徑線,不壓相鄰節點。
- 推的方向選「地圖外側較近的邊」(往空白邊,不往字塊那側);近水平的路段改往下推。
- 連接線從整數中點畫到節點。
- 驗:maxLv 6/7/8/11/12/21 六種曲線位置,故事節點離最近關卡邊緣 44~113px 全正(不重疊)。


## 2026-07-08(20 / Claude Opus 4.8)— 故事節點改「碰撞閃避」:同時避開關卡節點與字塊叢

### 使用者實玩(承上一筆)
上一筆改法線推出去,但地圖上還有「單字字塊叢」(一整叢在路徑某一側)—— 故事節點被推進 friend 字塊裡。地圖有三種東西搶空間:蜿蜒關卡路徑、字塊叢、故事節點。

### 修(js/10 mapSVG)
- **記錄字塊叢佔用範圍**:stage loop 裡把每階 badges 的實際矩形(clusterX + side 方向、rows×rowGap 高)推進 `chipBoxes`。
- **故事節點試多候選點挑最遠**:法線兩側 × 三段距離(D/1.6D/2.3D)+ 上下,共 8 個候選;`clearOf()` 算每點離「最近關卡節點 + 最近字塊矩形」的邊緣距離,選最大的。
- 驗(多個 maxLevel;量對「實際渲染的 badge rect」+ 關卡節點):使用者實際位置(第2階)關卡 61px / 字塊 93px(原本壓在 friend 上);全部階段無重疊。
- 已知邊界:深階(第41關 story_home)最擠 4px —— 路徑蜷太密、四面都貼關卡,已是候選最遠;仍不重疊,只是視覺偏近。第6階後才遇到,可接受。


## 2026-07-08(21 / Claude Opus 4.8)— 字塊叢改單欄釘邊緣:結構解決「換關卡壓到單字卡」

### 使用者實玩
換到新階(第3階)時,當前大關卡節點壓在字塊「開心 happy」上。挖座標確認:字塊排**兩欄**(欄距 208px)必然佔 ~380px 寬,關卡節點卡在兩欄中間 → 左右移都會撞一欄或出框(死結)。垂直閃避又打地鼠。★ Codex 已退訂 → 地圖版面回歸 Claude,這次做結構解不再半套。

### 修(js/10 mapTopicCluster + mapSVG)
- **字塊叢兩欄 → 單欄**:單欄只 ~184px 寬。
- **釘在地圖邊緣**:路徑本來就被限制在畫面 18%~82% 中央帶(`pt()` 的 clamp),字塊釘在左右邊緣(`clusterX = side>0 ? W-badgeHalfW-14 : badgeHalfW+14`)→ 路徑在中間、字塊在邊,**結構上天生不重疊**。高度隨字數,地圖本來就可垂直捲。
- 移除舊的 clusterHalfW(過大逼字塊置中)、badgeAnchorOffset、colGap、>4字特例、以及上一輪失敗的垂直掃描。chipBoxes 改用單欄範圍。
- 驗(掃 1~32 關):字塊 vs 關卡節點**零重疊**(全 ≥6px)、零出框、字塊貼著該階(第3階字塊 y≈1000 貼 L11 y=914)、故事節點不疊、不同階字塊不互疊。


## 2026-07-08(22 / Claude Opus 4.8)— 字塊叢加「錯落有致」zigzag(治單欄太醜)

### 使用者回饋
單欄釘邊緣擋到問題解了,但「美術上比較醜」(直挺挺一排像清單)。使用者選「錯落有致的散佈」方向。

### 修(js/10 mapTopicCluster)
- 一顆顆膠囊做 zigzag:奇數往邊緣外側凸 `side*zig`(24/20)、上下微錯 ±7 → 有機的一叢,不再是死板直排。
- **只往邊緣外側偏**(不往路徑那側)→ 不會再壓到關卡節點。
...
## 2026-07-08(25 / Claude Opus 4.8)— 重寫咖啡店故事:狗從店員改成搗蛋寵物

### 使用者
「餐廳是狗在送餐好奇怪」。同意 —— 狗當服務生不合理。定案:留狗當貫穿角色(偷蛋糕/藏貓/開口說都保留),只重寫階1咖啡店。

### 修(js/09 story_drink)
- 狗從「上錯飲料的店員」改成「搶先偷喝的寵物」:Sam 點咖啡→狗偷喝、點茶→狗也喝掉、點水→狗喝飽了不喝,Sam 終於喝到水。狗做的是狗會做的事(偷喝),不是上班。
- 標題「狗狗咖啡店」→「誰喝了我的咖啡?」,跟階4「誰吃了蛋糕?」對稱呼應(都是狗當犯人)。最後一題把「狗是店員」放成錯誤選項(自嘲舊設定)。
- 保留階1字彙(water/tea/coffee/sugar/with/please)+ 重組題 Coffee with sugar。地圖節點標題自動跟 STORIES.title。
- 驗:走完 4/4 題、+2金幣、無「店員/端來」殘留。


## 2026-07-08(26 / Claude Opus 4.8)— 長句排詞在寬螢幕折行往下捲:關卡舞台放寬到 1240

### 使用者實玩 + 提問
回應題(複合句 6 字塊)在寬螢幕還是折行、字卡被擠到要往下捲。使用者點破「左右很多空間沒必要限這麼窄」+ 問「不限寬其他螢幕會被切嗎」。

### 答/修
- 限寬不是怕螢幕被切(超寬只會留白),是怕字攤太開難讀。但 960 對複合句 6~7 字塊太緊 → 折行變高往下捲。
- css/01:#screen > .lesson max-width 960 → 1240(排詞/回應這類關卡舞台)。字塊是置中固定寬 → 放寬只防折行、不會攤太開。窄型答題(選擇/克漏/配對/教卡)各自 cap 960,computed 驗證未受影響。
- 驗(resize 1600/1024):回應題 slots+bank 各排成 1 排、作答帶不捲;短排句不受影響;選擇題/教卡 maxW 仍 960。


## 2026-07-08(27 / Claude Opus 4.8)— 補考題加「複習」標記(治「以為答錯沒再考」)

### 使用者提問 + 查證
「答錯的題最後不會重新考同一題?」→ 端到端測試證實:補考機制其實有運作(故意答錯2題,主回合跑完後兩題都被抓回來重考、同題型)。真正問題:補考題跟一般題長一模一樣、沒任何標示,玩家看不出答錯的回來了 → 誤以為沒補考。

### 修(js/01 + css/01)
- injectReviewBadge():補考題頂端插一條琥珀 pill「🔁 複習 · 剛剛答錯的,再來一次」。reviewThenAsk 兩條路徑(直接補考 / 連錯2次看完複習卡回來)都插。
- 驗:故意答錯2題,補考時看得到複習標記。


## 2026-07-08(28 / Claude Opus 4.8)— 分類題:題目搬到選項正上方(治「先看到選項沒看到要選什麼」)

### 使用者實玩
分類題的「題目」就是那句分類(選出所有感受或心情),但它在最上面、離選項很遠。視線聚焦在中間選項,常先看到 hungry/eat/bread/sad、沒讀到要選哪個分類。

### 修(js/05 + css/03)
- 分類句從最上面(shell prompt)搬到作答帶、選項正上方(.category-head,大而醒目);頂端 prompt 改成通用「分類題 · 可多選」。
- head+說明+選項一起置中 → 視線落在中間就直接看到「選什麼」。
- 驗:頂端=分類題·可多選、中間=選出所有感受或心情、順序 head→說明→選項。


## 2026-07-08(29 / Claude Opus 4.8)— 選意思題誘答亂碼「我(發問用)不喝水」

### 使用者實玩
選意思題(這句話在說什麼?)的誘答出現「我(發問用)不喝水」——「發問用」是 do 的中文標註漏進「逐字直翻」誘答,看起來像亂碼。
...
## 2026-07-08(31 / Claude Opus 4.8)— 拿掉「看2秒→默寫」(flashtype)

### 使用者
「先看幾秒再寫這題是不是不需要?」認同:flashtype(看字2秒→消失→憑記憶拼)是純死背字母序,跟聽寫(音↔拼)/看圖寫(意思↔拼)重疊,最不「讓英文有道理」,強迫等2秒體感也卡。
...
## 2026-07-08(33 / Claude Opus 4.8)— 兩個新句子題型:看中文說英文 + 聽英文選中文

### 使用者
想強化中↔英雙向的理解與連結。(並提醒專案已被平行編輯:新增了 blueprint 系統(日常L1-3固定題序)+ sentence_answer(一問一答),已重新讀過跟上。)

### 加(js/06 + js/08)
- **sentence_say 看中文→說英文句**(skill speak, tier2):只給中文,自己用英文說出來 → 按「看正解」露出英文+自評(整句語音辨識不準 → 自評式不硬判不扣分)。比跟讀(給英文照念)難一階=真口說產出。掛 ARRANGE_FAMILY 共用冷卻。
...
## 2026-07-08(36 / Claude Opus 4.8)— 快測模式(dev.fast):整條曲線玩得完不燒到心

### 使用者糾正
跳關工具沒用——要測的是「實際玩起來順不順、學習曲線好不好、哪裡要補強」,跳過去就測不到。真正的痛:評估整條曲線得從頭玩,但一輪太長玩到25關就沒電,後半永遠測不到。

### 加(js/01 buildLessonQuota + js/11 dev.fast)
- 快測模式:每字只出必要題數(新字教+考2、其餘1)→ 一輪從約100題/12關降到50,整條55關約20分鐘玩得完。
- **照順序玩完每一階**,新題型/難度台階/情境卡/故事節點照樣遇到 → 真的在測曲線,不是跳過。
- dev.fast() 開 / dev.fast(false) 關,存 localStorage(刷新後還在)。玩家碰不到。
- 驗:正常97題→快測50題(12關)、新字關照樣教考、開關持久。
