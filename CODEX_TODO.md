...
> Claude(負責引擎/內容)以前這台瀏覽器視窗壞掉(2px),**2026-07-05 已修:現在量得到像素**(`getBoundingClientRect`/`preview_inspect` 通),只有拍真圖仍逾時。所以下面的版面活 Claude 能自驗對錯了;交給你的主要是**純美感 / 好不好看**那層,以及需要真的看一眼畫面的地圖微調。
>
...
## 7.（使用者定案）劃重點標註 · 視覺規範
統一記號視覺(**顏色/線寬 Codex 定**,原則:深色底上清爽、不吵、不像方塊):
- **全部用「虛線底線」** —— 不是實線、更**不是整個字變色 / 填色塊**。文字保持原色,只在下面加一條**虛線**。
- **橘色虛線 = 沒教過的詞**。★ **只在閱讀/沉浸情境用**;**教卡不要套**(教卡每個字本來就是新的,套了沒意義又混淆)。
- **淺色虛線 = 有註解的詞**。
- **劃重點色(霧藍=拼讀 / 琥珀=字根)= 字裡的那一段**(色帶或底線,抓順眼即可)。
- **skip-ink**:底線在 g / j / p / q / y 會斷掉 → 決定要不要 `text-decoration-skip-ink: none` 讓它連續。
- **記號說明(還沒做)**:第一次出現教一次,或設定裡放個小圖例(橘虛線=沒教過、淺虛線=有註解、劃重點色=字根/拼讀)。**說明文字 Claude 出。**
- **顯示範圍**:只在 **教卡 / 答錯回饋 / 複習卡**;**答題選字 UI 不顯示**(選項 `js/03:154`、`js/05:60,103`;排詞字塊 `js/06:12,23,87`;快閃 `js/05:327`;特訓挑字清單 `js/10:273` → 換回純文字)。
