...
function teach(w) {
  ...
  const teachPic = typeof visualOf === 'function' && visualOf(w) ? `<div class="teach-pic">${picHTML(w, 72)}</div>` : '';   // 有圖的字:圖跟單字同一排(疊直的會撐爆固定一頁的教卡);沒圖不硬放
  ...
  shell('先認識這個字 👀', `
    <div class="teach-layout">
      <div class="teach-main${ftp ? ' has-phonics' : ''}">
        <div class="teach-word-row">
          ${teachPic}
