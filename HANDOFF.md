...
### 接下來(跟使用者定案的順序)
1. **「學會」門檻改嚴(隔關再驗)**:使用者點破「默寫過一次≠會(失敗10次硬過一次只是短期記憶)」。定案規則:第一次默寫成功只記 `wroteClock` 不算數;**下一關以後** SRS 再抽一次寫題過了才蓋「真的會寫」章,階段門檻認這個章;沒過就繼續掛補寫佇列。(補考答對不算 wrote 的既有防線不動。)
2. **Step 2 動力迴圈**:情境完成卡(階段收尾「你會點飲料了 ☕」+ 該批句子展示 + 金幣動畫)、過關結算熟練度條動畫、答題連擊。治「主線等速直線沒高潮」(王移去小遊戲後階段收尾沒有任何時刻)。
3. **Step 3 關卡畫面微整**:選項配圖、答對回饋動畫、地圖標情境名(節點=情境)。
