...
### ★ 環境限制(決定分工,重要)
**這台給 Claude 的瀏覽器視窗是壞的:寬 2px、截圖逾時**(這輪當場證實)。→ **Claude 只能 `eval` 讀 DOM/驗邏輯,看不到版面長相**。**版面/像素/地圖 → Codex(看得到真螢幕)或靠使用者截圖**;引擎/內容/邏輯 Claude 做、console 驗。
...
### 實作切法(建議)
1. **資料層(Claude)**:拼讀規則表(`match` 正則偵測字首/字尾 → label/念法)、字根表(漸進補)、`annotate(word)` 回「這字標哪些段、各是什麼類、註解是什麼」;`isLearned`→是否隱藏;答錯→挑相關那條進 `.why`。
...
### 版面(★ 給下一個:這塊我在此環境「看不到像素」→ 建議交給 Codex)
- 這環境**截圖逾時、量位置回傳 0(0×0 viewport)**,所以我調版面是**閉眼猜、靠使用者截圖回饋** → 打地鼠、很耗使用者。**純像素/CSS 的活最好交給看得到畫面的 Codex**(這也是使用者原本規劃:內容/引擎歸 Claude、UI 歸 Codex)。
